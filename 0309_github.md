...
### 컴에서 git 계정 로그인되어있는지 확인하기

```제어판 > 사용자 계정 > 자격 증명 관리자 > windows 자격 증명 > 자격증명```에서 목록에 없으면 로그인되어있지 않은거 !

### 컴에서 git 계정 로그아웃하기

위 경로에서 ```> 일반 자격증명```에서 git ```제거```
...
## 로컬폴더에 연동된 git url 확인하기

git repository를 여러개 사용하다보면 어떤 로컬폴더가 어떤 git repository로 연동이 되는지 모를 때가 있어. 그럴 때 bash에서 아래의 코드를 삽입하면 알 수 있어.
